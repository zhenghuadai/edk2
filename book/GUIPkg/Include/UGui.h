...
public:
    Window():Brick(false),mBrickList(0), mTabList(0), mEventHandler(0),mbgBmp(0){Brick::CurWindow = this;}
...
public:
    ...
    Label(void* handle, UINT16 strid):mStringId(strid),mHiiHandle(handle){}
public:
    virtual void DrawTo(void* pFrameBuffer, UINT32 ToScreen = 0);
private:
    UINT16 mStringId;
...
{
public:
    Button(CHAR16* str, CHAR8* bg = 0):Label(str),mbackGround(0),bShowFrame(TRUE){SetBackGroundFile(bg); Size(100,30);}
    Button(void* handle, UINT16 strid, CHAR8* bg = 0):Label(handle,strid),mbackGround(0),bShowFrame(TRUE){SetBackGroundFile(bg);}
...
public:
    ...
    void TurnOn();
    void TurnOff();
    Switcher(const CHAR8* on, const CHAR8* off):mstate(SWITCH_ON),mbmpon(0), mbmpoff(0){
